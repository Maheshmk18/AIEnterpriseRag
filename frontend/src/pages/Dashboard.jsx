...
function Dashboard() {
    ...
    useEffect(() => {
        ...
        if (userData) {
            ...
            // OPTIMIZATION: Redirect non-admins immediately to avoid unnecessary data fetching
        ...
        try {
            ...
            if (isAdmin()) {
                try {
                    ...
                    console.error('Could not load users count:', e);
                }
            }
        } catch (error) {
            console.error('Failed to load data:', error);
            // If unauthorized, the interceptor handles it, but we should reset loading
        } finally {
            setLoading(false);
        }
    };
    ...
    if (!isAdminUser) {
        // Redirect logic is handled in useEffect to avoid warning
        return null;
    }
